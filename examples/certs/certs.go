...
// ── In-memory PKI + copy-paste bundle ──────────────────────────────
//
// The filesystem helpers above suit a shared machine. When you would
// rather NOT touch disk — hand the client a credential to paste — use
// a PKI: it keeps the CA in memory, and ClientBundle produces a single
// base64 token (CA + client cert + key) the server can print and the
// client can consume from a flag.

// PKI is an in-memory certificate authority for one demo run.
type PKI struct {
...
// NewPKI generates a fresh in-memory CA.
...
// Pool returns a cert pool trusting this CA — used to verify the other
// side's certificate.
...
// ServerCert issues a leaf certificate for name, for the hub to
// present on its TLS listener.
func (p *PKI) ServerCert(name string) (tls.Certificate, error) {
	return p.leaf(name)
}

// ClientBundle issues a leaf for name and packages it with the CA into
// a Bundle the client can load without touching disk.
func (p *PKI) ClientBundle(name string) (Bundle, error) {
	der, key, err := newLeaf(name, p.caCert, p.caKey)
	if err != nil {
		return Bundle{}, err
	}
...
func (p *PKI) leaf(name string) (tls.Certificate, error) {
	der, key, err := newLeaf(name, p.caCert, p.caKey)
	if err != nil {
		return tls.Certificate{}, err
	}
...
// Bundle is a client's credential set as PEM: the CA to trust plus the
// client's own certificate and key. Encode it to a copy-pasteable
// token; DecodeBundle reverses it.
type Bundle struct {
	CA   []byte `json:"ca"`
	Cert []byte `json:"cert"`
	Key  []byte `json:"key"`
...
// Encode returns a single-line base64 token carrying the bundle.
...
// DecodeBundle parses a token produced by Bundle.Encode.
func DecodeBundle(token string) (Bundle, error) {
	raw, err := base64.StdEncoding.DecodeString(token)
	if err != nil {
		return Bundle{}, fmt.Errorf("certs: token is not valid base64: %w", err)
	}

	var b Bundle
	if err := json.Unmarshal(raw, &b); err != nil {
		return Bundle{}, fmt.Errorf("certs: token payload is invalid: %w", err)
	}

	return b, nil
}

// ClientTLS builds a mutual-TLS client config from the bundle: present
// our cert, verify the server's cert (with the expected serverName)
// against the bundled CA.
func (b Bundle) ClientTLS(serverName string) (*tls.Config, error) {
	cert, err := tls.X509KeyPair(b.Cert, b.Key)
	if err != nil {
		return nil, fmt.Errorf("certs: bundle key pair: %w", err)
	}

	pool := x509.NewCertPool()
	if !pool.AppendCertsFromPEM(b.CA) {
		return nil, fmt.Errorf("certs: bundle CA is invalid")
	}
