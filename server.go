...
// is the one-call counterpart of NewClient for the server side — the
// pieces underneath (hub.NewRegistry, hub.NewHandler, proxy.New)
// stay public for applications that want to mount them on their own
// routers.
//
//	srv := holt.NewServer(
...
// hub.NewRegistry(logger) — the way to set a hub id, a shared presence
// Directory, or a MeterProvider.
func WithRegistry(registry *hub.Registry) Option {
	return serverOption(func(s *Server) { s.registry = registry })
}
...
func NewServer(opts ...Option) *Server {
	...
	if s.registry == nil {
		s.registry = hub.NewRegistry(s.logger)
	}
...
func (s *Server) start(ctx context.Context) ([]*http.Server, error) {
	...
	if s.tunnel != nil && s.tunnel.configured() {
		...
		handler := middleware(hub.NewHandler(s.registry, identity, s.logger, s.tunnel.handlerOpts...))
