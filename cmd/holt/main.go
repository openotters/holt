...
//	holt block <peer>            # disconnect + revoke the credential
...
func main() {
	...
	kctx.FatalIfErrorf(kctx.Run(cli.Commons))
}
...
type CMD struct {
	*c.Commons
	...
	Block   *commands.Block   `cmd:"" help:"Block a peer's credential and close its tunnel (it cannot reconnect)."`
	Unblock *commands.Unblock `cmd:"" help:"Lift a peer's block."`
}
