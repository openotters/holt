...
// the client side — a thin veneer over dial.Run, which stays public
// for applications that want the raw loop.
//
...
type ClientOption interface{ applyClient(*Client) }

// clientOption implements ClientOption.
...
func WithKeepalive(every time.Duration) ClientOption {
...
func WithTunnelTLS(cfg *tls.Config) ClientOption {
